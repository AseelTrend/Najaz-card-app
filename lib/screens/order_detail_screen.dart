...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  Widget _timelineCard() {
    return _panel('سجل الحالة', Column(children: _log.map((item) {
      ...
      return ListTile(contentPadding: EdgeInsets.zero, leading: Container(width: 12, height: 12, decoration: BoxDecoration(color: color, shape: BoxShape.circle)), title: Text(_statusLabel(status), style:  TextStyle(color: AppColors.text, fontSize: 13, fontWeight: FontWeight.bold)), subtitle: Text(item['created_at']?.toString() ?? '', style:  TextStyle(color: AppColors.text3, fontSize: 11)), trailing: Text(item['message']?.toString() ?? '', style:  TextStyle(color: AppColors.text2, fontSize: 11)));
    }).toList()));
  }
  ...
  Widget _panel(String title, Widget child) => Container(margin: const EdgeInsets.only(top: 14), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(17), border: Border.all(color: AppColors.border)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style:  TextStyle(color: AppColors.text, fontSize: 15, fontWeight: FontWeight.bold)), const SizedBox(height: 8), child]));

  Widget _infoRow(String label, String value) => Padding(padding: const EdgeInsets.symmetric(vertical: 6), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Text(label, style:  TextStyle(color: AppColors.text2, fontSize: 12))), const SizedBox(width: 12), Flexible(child: Text(value, textAlign: TextAlign.end, style:  TextStyle(color: AppColors.text, fontSize: 12, fontWeight: FontWeight.w600)))]));
}
