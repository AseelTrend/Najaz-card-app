...
// [FIX] لا تُقتل السكربت لو انقطع الاتصال الذي شغّله (مثل self-ping الصفحة الإدارية
// الذي يغلق الاتصال فوراً بعد الإرسال) — بدون هذا قد تموت المزامنة عند أول echo
...
// ══════════════════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════════════════
// [FIX] استخراج سبب الرفض/الحالة من replay_api — الحقل الموثّق فعلياً في وثائق
// ap4stor (وليس أي من reject_reason/cancel_reason/... التي لا وجود لها في الرد
// الحقيقي). شكل الحقل غير ثابت في أمثلة التوثيق نفسها (أحياناً مصفوفة نصوص
// مباشرة، وأحياناً مصفوفة كائنات فيها مفتاح replay) لذا نتعامل مع الحالتين.
// ══════════════════════════════════════════════════════════════════════════════
...
function checkOrderWithProvider(PDO $pdo, array $ord): bool {
    ...
    if ($type === 'numbersapp') {
        return syncNumbersAppOrder($pdo, $ord);
    }
...
// NumbersApp
// ══════════════════════════════════════════════════════════════════════════════
function syncNumbersAppOrder(PDO $pdo, array $ord): bool {
    $orderId   = $ord['id'];
    $apiKey    = $ord['prov_key'];
    $baseUrl   = rtrim($ord['prov_url'], '/');
    $provOrderId = $ord['provider_order_id'];

    // قراءة بيانات NumbersApp المحفوظة
    $naData = [];
    try {
        $ndStmt = $pdo->prepare("SELECT numbersapp_data FROM orders WHERE id=?");
        $ndStmt->execute([$orderId]);
        $ndRaw = $ndStmt->fetchColumn();
        if ($ndRaw) $naData = json_decode($ndRaw, true) ?? [];
    } catch(Exception $e) {}
    ...
    // ── Live: فحص الكود ────────────────────────────────
    if ($naType === 'live') {
        $url = $baseUrl . '/checkForCode?' . http_build_query([
            'api_key' => $apiKey,
            'id'      => $provOrderId,
        ]);
        $response = httpGet($url, []);
        if (!$response) { log_msg("⚠️  فشل الاتصال بـ NumbersApp Live #{$orderId}"); return false; }

        $data = json_decode($response, true);
        if (!$data) return false;

        $status = $data['status'] ?? '';
        if ($status === 'STATUS_OK' && isset($data['code'])) {
            // تم استلام الكود
            $naData['code'] = $data['code'];
            $noteText = 'NUMBERSAPP_LIVE: code=' . $data['code'] . ' | number=' . ($naData['number'] ?? '');
            $pdo->prepare("UPDATE orders SET numbersapp_data=? WHERE id=?")->execute([json_encode($naData, JSON_UNESCAPED_UNICODE), $orderId]);
            updateOrderStatus($pdo, $ord, 'completed', $noteText);
            log_msg("✅ NumbersApp Live #{$orderId} → completed (code={$data['code']})");
            return true;
        }
        // لا يزال ينتظر
        return false;
    ...
    // ── Pack: فحص حالة الباقة ──────────────────────────
    if ($naType === 'pack_fixed' || $naType === 'pack_count') {
        $url = $baseUrl . '/checkPackRequestState?' . http_build_query([
            'api_key' => $apiKey,
            'Id'      => $provOrderId,
        ]);
        $response = httpGet($url, []);
        if (!$response) { log_msg("⚠️  فشل الاتصال بـ NumbersApp Pack #{$orderId}"); return false; }

        $data = json_decode($response, true);
        if (!$data || ($data['res'] ?? '') !== 'Success') return false;

        $state = $data['boughtItem']['State'] ?? '';
        $msg   = $data['boughtItem']['msg'] ?? '';

        if ($state === 'Done') {
            $naData['state'] = 'Done';
            $pdo->prepare("UPDATE orders SET numbersapp_data=? WHERE id=?")->execute([json_encode($naData, JSON_UNESCAPED_UNICODE), $orderId]);
            updateOrderStatus($pdo, $ord, 'completed', 'NUMBERSAPP_PACK: Done');
            log_msg("✅ NumbersApp Pack #{$orderId} → completed");
            return true;
        }
        if ($state === 'Refused') {
            $naData['state'] = 'Refused';
            $naData['refuse_msg'] = $msg;
            $pdo->prepare("UPDATE orders SET numbersapp_data=? WHERE id=?")->execute([json_encode($naData, JSON_UNESCAPED_UNICODE), $orderId]);
            updateOrderStatus($pdo, $ord, 'cancelled', 'NUMBERSAPP_PACK: Refused' . ($msg ? " — $msg" : ''), $msg);
            log_msg("❌ NumbersApp Pack #{$orderId} → refused: {$msg}");
            return true;
        }
        // Waiting — لا تغيير
        return false;
    ...
    // ── Store: المنتجات تُسلَّم فوراً لا حاجة لمزامنة ──
    if ($naType === 'store') {
        return false; // مكتملة من البداية
    }

    log_msg("⚠️  NumbersApp نوع غير معروف: {$naType} (طلب #{$orderId})");
    return false;
}
...
function syncOranosOrder(PDO $pdo, array $ord): bool {
    ...
    // [FIX] replay_api هو الحقل الموثّق فعلياً لدى ap4stor لرسالة المزود —
    // بقية الحقول أدناه لم تكن موجودة أصلاً في أي رد حقيقي فتبقى fallback فقط
...
function updateOrderStatus(PDO $pdo, array $ord, string $newStatus, string $noteText, string $provNote = ''): void {
    ...
    try {
        ...
        // [FIX] قفل ذري: بدل الاعتماد على $ord['status'] (نسخة قديمة أُخذت قبل
        // انتظار رد المزود)، نحاول "حجز" الطلب بتحديث شرطي الآن. إن لم يتأثر
        // أي صف فهذا يعني أن سكربت مزامنة آخر (sync_order.php / admin cron /
        // api/cron.php) عالج نفس الطلب بالفعل — نتوقف فورًا بلا استرداد مكرر.
        if (in_array($newStatus, ['cancelled', 'failed'])) {
            $claim = $pdo->prepare("
                UPDATE orders SET status=?
                WHERE id=? AND status NOT IN ('completed','cancelled','failed')
            ");
            $claim->execute([$newStatus, $orderId]);
            if ($claim->rowCount() === 0) {
                $pdo->rollBack();
                log_msg("⏭️  تخطي الطلب #{$orderId}: عولج بالفعل من عملية موازية");
                return;
            }
