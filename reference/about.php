...
$managedPage = sitePagesFindVisibleBySlug($pdo, 'about');
$pageTitle = (($managedPage['title'] ?? 'من نحن') . ' — ' . $siteName);
$content = $managedPage['content'] ?? (getSetting('page_about') ?: '<p>هذه الصفحة قيد التحديث.</p>');
$isLoggedIn = isLoggedIn();
$userBalance= 0;
if($isLoggedIn){ $u=$pdo->prepare("SELECT balance FROM users WHERE id=?"); $u->execute([$_SESSION['user_id']]); $u=$u->fetch(); $userBalance=$u['balance']??0; }
?>
<!DOCTYPE html>
...
<style>
:root{--bg:#080c1a;--bg2:#0d1428;--card:#111827;--card2:#1a2340;--border:rgba(255,255,255,.07);--primary:#1e6fff;--cyan:#00d4ff;--green:#00e676;--text:#fff;--text2:#8fa3bf;--text3:#4d6080;--font:'Cairo',sans-serif;--r:18px}
...
/* Page hero */
.page-hero{background:linear-gradient(135deg,#0d1f5c 0%,#080c1a 100%);padding:28px 20px 24px;text-align:center;border-bottom:1px solid var(--border);position:relative;overflow:hidden}
.page-hero::before{content:'';position:absolute;top:-40px;right:-40px;width:160px;height:160px;background:radial-gradient(circle,rgba(30,111,255,.18) 0%,transparent 70%);pointer-events:none}
.page-hero-icon{width:64px;height:64px;background:linear-gradient(135deg,rgba(30,111,255,.25),rgba(0,212,255,.15));border:2px solid rgba(30,111,255,.3);border-radius:20px;display:flex;align-items:center;justify-content:center;font-size:1.8rem;margin:0 auto 14px;position:relative;z-index:1}
...
.html-content blockquote{border-right:3px solid var(--primary);padding:10px 14px;background:rgba(30,111,255,.06);border-radius:0 8px 8px 0;margin:12px 0;color:var(--text2)}
...
<div class="page-hero">
  ...
  <div class="page-hero-title">من نحن</div>
