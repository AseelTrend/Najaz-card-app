...
try {
    ...
    if (!$rate) jsonOutMobile(false, 'عملة غير معتمدة');

    $amountUSD = $amountSent * (float)$rate['rate_to_usd'];
    ...
    if (!empty($_FILES['receipt']['name']) && $_FILES['receipt']['error'] === UPLOAD_ERR_OK) {
        ...
        if ((in_array($file['type'], $allowedTypes) || in_array($ext, $allowedExts)) && $file['size'] <= 5 * 1024 * 1024) {
            ...
            if (!is_dir($dir) && !mkdir($dir, 0755, true)) jsonOutMobile(false, 'تعذر تجهيز مجلد الإيصالات');
            $filename = 'receipt_' . $userId . '_' . time() . '.' . $ext;
            if (!move_uploaded_file($file['tmp_name'], $dir . $filename)) jsonOutMobile(false, 'تعذر حفظ الإيصال');
    ...
    error_log('Mobile manual topup failed: ' . $e->getMessage());
    jsonOutMobile(false, 'تعذر حفظ طلب الشحن، تحقق من إعدادات العملة ووسيلة الدفع');
}
