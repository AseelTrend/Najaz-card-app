...
foreach ($orders as $order) {
    ...
    if ($result && isset($result['status'])) {
        ...
        if ($newStatus !== $order['status']) {
            // [FIX] قفل ذري: هذا سكربت كرون قديم بلا أي transaction، وقد يعمل
            // بنفس لحظة cron_sync_orders.php أو sync_order.php على نفس الطلب.
            // التحديث الشرطي هنا (WHERE status NOT IN ...) يضمن أن استرداد
            // الرصيد يحدث مرة واحدة فقط حتى لو تزامن أكثر من سكربت.
            $claim = $pdo->prepare("
                UPDATE orders SET status=?, updated_at=NOW()
                WHERE id=? AND status NOT IN ('completed','cancelled','failed')
            ");
            $claim->execute([$newStatus, $order['id']]);
