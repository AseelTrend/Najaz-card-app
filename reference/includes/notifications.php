...
/**
 * يحدد ما إذا كانت الإدارة تسمح بإرسال تحديثات العملاء عبر Telegram.
 * عند غياب جدول/عمود الإعداد في تثبيت قديم، نسمح بالإرسال حفاظًا على التوافق.
 */
function telegramCustomerUpdatesEnabled($pdo): bool {
    try {
 ...
 */
function sendTelegramCustomerNotification($pdo, $userId, $type, $title, $message, $referenceType = null, $referenceId = null) {
    // الطلبات والشحن لها قنوات مخصصة أسفل الملف؛ لا نكررها هنا.
    if (in_array((string)$type, ['order_update', 'topup'], true)) return false;
    try {
        if (!telegramCustomerUpdatesEnabled($pdo)) return false;
        ...
        $keyboard = null;
        if ((string)$referenceType === 'order' && (int)$referenceId > 0) {
            $keyboard = [[['text' => '🔎 عرض الطلب', 'callback_data' => 'order:view:' . (int)$referenceId]], [['text' => 'الرئيسية', 'callback_data' => 'home']]];
        } elseif ((string)$referenceType === 'wallet') {
            $keyboard = [[['text' => '🧾 مدفوعاتي', 'callback_data' => 'menu:payments']], [['text' => 'الرئيسية', 'callback_data' => 'home']]];
        }
        return (bool)sendTelegram($pdo, $chatId, "🔔 <b>{$safeTitle}</b>\n{$safeMessage}", $keyboard);
    } catch (Throwable $e) {
...
function sendNotification($pdo, $userId, $type, $title, $message, $icon = 'bell', $color = '#6c3fe0', $refType = null, $refId = null) {
    try {
        ...
        sendTelegramCustomerNotification($pdo, $userId, $type, $title, $message, $refType, $refId);
        return true;
    } catch (Exception $e) {
        return false;
    }
...
function getActiveBroadcastsForUser($pdo, $userId) {
    try {
        ...
        $rows = $pdo->prepare("
            ...
            LEFT JOIN broadcast_views bv ON bv.broadcast_id = nb.id AND bv.user_id = ?
            WHERE nb.is_active = 1
              AND (nb.end_at IS NULL OR nb.end_at > ?)
              AND (nb.target = 'all' OR (nb.target = 'specific' AND JSON_CONTAINS(nb.target_user_ids, CAST(? AS JSON))))
              AND (nb.show_once = 0 OR bv.id IS NULL)
            ORDER BY nb.created_at DESC
            LIMIT 3
        ");
        $rows->execute([$userId, $now, $userId]);
 ...
 * إرسال رسالة تليغرام
 */
function sendTelegram($pdo, $chatId, $text, $keyboard = null) {
    $token = getSetting('telegram_bot_token');
    if (!$token || !$chatId) return false;
    $url  = "https://api.telegram.org/bot{$token}/sendMessage";
    $data = ['chat_id' => $chatId, 'text' => $text, 'parse_mode' => 'HTML'];
    if (is_array($keyboard) && $keyboard) {
        if (function_exists('njazTgApplyMenuIcons')) $keyboard = njazTgApplyMenuIcons($pdo, $keyboard);
        if (function_exists('njazTgStyledKeyboard')) $keyboard = njazTgStyledKeyboard($keyboard);
    ...
    $err = curl_error($ch);
    curl_close($ch);
    if ($err || $res === false) return false;
    $decoded = json_decode((string)$res, true);
    return is_array($decoded) ? !empty($decoded['ok']) : false;
}
...
function notifyNewOrder($pdo, $order, $user, $serviceName) {
    ...
    if (telegramCustomerUpdatesEnabled($pdo) && !empty($user['telegram_chat_id']) && $user['notif_order_update']) {
        $msg = "🛍️ تم استلام طلبك {$orderRef}\n"
             . "الخدمة: {$serviceName}\n"
             . "الحالة: قيد الانتظار ⏳";
        sendTelegram($pdo, $user['telegram_chat_id'], $msg, [[['text' => '🔎 عرض الطلب', 'callback_data' => 'order:view:' . $orderId]], [['text' => '📦 طلباتي', 'callback_data' => 'menu:orders']]]);
    }
...
function notifyOrderStatusChange($pdo, $order, $newStatus, $adminNotes = '') {
    ...
    // 3. تليغرام للعميل
    if ($user && telegramCustomerUpdatesEnabled($pdo) && !empty($user['telegram_chat_id']) && $user['notif_order_update']) {
...
function notifyTopupApproved($pdo, $userId, $amount, $currency) {
    ...
    if (telegramCustomerUpdatesEnabled($pdo) && !empty($user['telegram_chat_id']) && $user['notif_topup_update']) {
        sendTelegram($pdo, $user['telegram_chat_id'], "💰 تم شحن رصيدك\nالمبلغ: {$amount} {$currency}\n✅ تمت الموافقة", [[['text' => '🧾 مدفوعاتي', 'callback_data' => 'menu:payments']], [['text' => 'الرئيسية', 'callback_data' => 'home']]]);
    }
...
function notifyTopupRejected($pdo, $userId, $amount, $currency, $reason = '') {
    ...
    if (!$user) return;
    if (telegramCustomerUpdatesEnabled($pdo) && !empty($user['telegram_chat_id']) && $user['notif_topup_update']) {
        sendTelegram($pdo, $user['telegram_chat_id'], "❌ تم رفض طلب الشحن\nالمبلغ: {$amount} {$currency}" . ($reason ? "\nالسبب: {$reason}" : ''), [[['text' => '🧾 مدفوعاتي', 'callback_data' => 'menu:payments']], [['text' => 'الرئيسية', 'callback_data' => 'home']]]);
    }
    if (!empty($user['whatsapp_number']) && $user['notif_topup_update']) {
        sendWhatsApp($pdo, $user['whatsapp_number'], "{$siteName}\n❌ تم رفض طلب الشحن\nالمبلغ: {$amount} {$currency}");
    }
