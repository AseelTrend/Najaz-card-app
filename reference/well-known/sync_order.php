...
try {
    ...
    // [FIX] قفل ذري: هذا السكربت يُستدعى فور تقديم الطلب (fire-and-forget) وقد
    // يتزامن مع cron_sync_orders.php. التحديث الشرطي هنا يمنع الاسترداد مرتين
    // لنفس الطلب — إن لم يتأثر أي صف فعملية موازية سبقتنا فنتوقف فورًا.
    $refunded = false;
    if (in_array($newStatus, ['cancelled', 'failed'])) {
        ...
        if ($claim->rowCount() === 0) {
            $pdo->rollBack();
            echo json_encode(['ok' => true, 'changed' => false, 'status' => $newStatus,
                'msg' => 'عولج بالفعل من عملية موازية']);
            exit;
        }
